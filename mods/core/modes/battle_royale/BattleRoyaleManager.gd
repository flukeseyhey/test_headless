...
#		var is_match: bool = score.get_score(winner_id) >= config['points_to_win']
		var is_match: bool = score.get_score(winner_id) >= 1
...
remotesync func show_winner(winner_name: String, host_score: Dictionary, is_match: bool = false) -> void:
	if is_match:
		...
		ui_layer.show_message(winner_name + " ชนะในรอบนี้!")
...
#	yield(get_tree().create_timer(2.0), "timeout")
	...
	get_tree().change_scene("res://src/main/match/RoundScreen.tscn")
