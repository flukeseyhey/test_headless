	...
	if use_teams:
		...
		for player_id in players:
			...
			if is_network_master():
				hud.score.rpc("update_medal", player.index)
	...
	if get_tree().is_network_server() and not game_over:
		if killer_id != -1 and not game_over:
			if use_teams:
				...
				var player_score = score.find_highest()
			
#				hud.score.rpc("set_highest_score", player_index)
#				hud.score.rpc("update_medal")
...
remotesync func show_winner(winner_name: String, host_score: Dictionary) -> void:
	if not instant_death:
		ui_layer.show_message(winner_name + " เป็นผู้ชนะ!")
	
	yield(get_tree().create_timer(2.0), "timeout")
	
	Music.stop()
	DbSystem.host_score = {score = host_score}
	scene = get_tree().change_scene("res://src/main/match/RoundScreen.tscn")
#	var round_scene = DbSystem.RoundScreen.instance()
