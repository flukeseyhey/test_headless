...
const DEFAULT_MODE = "res://mods/core/modes/deathmatch.tres"

#-------------------------------------------------------------- RANDOM MODE
const BATTLE_ROYALE_MODE = "res://mods/core/modes/battle_royale.tres"
const DEATHMATCH_MODE = "res://mods/core/modes/deathmatch.tres"
#------------------------------------------------------------------------
...
func _ready() -> void:
	...
	var modes := [BATTLE_ROYALE_MODE, DEATHMATCH_MODE]
	var random_mode = modes[randi() % modes.size()]

	_load_match_modes()
	for match_mode_id in match_modes:
		mode_field.add_item(match_modes[match_mode_id].name, match_mode_id)
	
	match DbSystem.set_enable_online_mode:
		0:
			mode_field.value = random_mode
		1:
			mode_field.value = BATTLE_ROYALE_MODE
		2:
			mode_field.value = DEATHMATCH_MODE
...
#	set_config_values(config_values)
	...
	set_config_values(get_config_values())
