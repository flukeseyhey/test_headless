...
func _ready() -> void:
	...
	if OnlineMatch.players.size() < 2:
		scene = get_tree().change_scene("res://src/main/title/StartScreen.tscn")
		return
	...
	_on_ReadyScreen_ready_pressed()
	DbSystem.game_is_playing = true
	DbSystem.total_online_bet(DbSystem.CurrentRoomType)
...
func _on_ReadyScreen_ready_pressed() -> void:
	...
	if RachaAPI.is_show_debug:
		print("manager_path : ", mode_screen.get_mode_manager_scene_path())
		print("config : ", mode_screen.get_config_values())
		print("map_path : ", map_screen.get_map_scene_path())
		print("teams : ", team_screen.get_teams())
	####################################################################################################################
