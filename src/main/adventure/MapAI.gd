...
func initialize(respawn_points: Array, pathfinding: Pathfinding):
	if respawn_points.size() == 0 or unit == null:
...
func handle_unit_death():
	match AdventureMatch.adventure_mode:
		...
		AdventureMatch.AdventureMode.LEVEL:
			GlobalSignals.emit_signal("unit_death")


#func set_unit_ai_to_advance_to_next_base(unit: ActorOffline):
#	if target_base != null:
#		var ai: AI = unit.ai
...
func _on_RespawnTimer_timeout() -> void:
	if AdventureMatch.adventure_mode == AdventureMatch.AdventureMode.ENDLESS:
