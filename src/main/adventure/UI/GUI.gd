...
func set_player(player: PlayerOffline):
	self.player = player

	set_new_health_value(player.health_stat.player_health)
...
func set_new_health_value(new_health: int):
	var original_color = Color("#04ff00")
	var highlight_color = Color("#ff0000")
